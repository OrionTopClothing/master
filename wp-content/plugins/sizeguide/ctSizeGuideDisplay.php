...
class ctSizeGuideDisplay {
			...
			if ( ! empty( $terms ) ) {
				foreach ( $terms as $term ) {
					if ( $child_id = $this->hasChild( $term->term_id, $terms ) ) {
						$sg_post_id_candidate = get_term_meta( $child_id, '_ct_assignsizeguide', true );
						if ( $sg_post_id_candidate != '' && get_post( $sg_post_id_candidate ) ) {
							$sg_post_id = $sg_post_id_candidate;
							break;
						}
					} else {
						$sg_post_id_candidate = get_term_meta( $term->term_id, '_ct_assignsizeguide', true );
						if ( $sg_post_id_candidate != '' && get_post( $sg_post_id_candidate ) ) {
							$sg_post_id = $sg_post_id_candidate;
						}
					}
	 ...
	 * @return bool
	 */
	public function hasSizeGuide( $post_id = null ) {
