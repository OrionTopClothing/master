...
class WOO_Slg_Public
{
    ...
    public function __construct()
    {
        ...
        $woo_slg_social_windowslive, $woo_slg_social_vk, $woo_slg_social_instagram, $woo_slg_social_amazon, $woo_slg_social_paypal,$woo_slg_social_line,$woo_slg_social_apple;
    ...
    public function woo_slg_social_login()
    {
        ...
        if ($type == 'facebook') {
            ...
            $gp_userdata = isset( $_POST["gp_userdata"] ) ? $_POST["gp_userdata"] : array();
        ...
        }else if ($type == 'line') { //check type is Line
        ...
        }else if ($type == 'apple') { //check type is Apple
    ...
    public function woo_slg_login_email()
    {
        ...
        if ( isset( $_POST['email']  ) && !empty($_POST['email'])) {
            ...
            $resendit = isset( $_POST['resendit'] ) ? $_POST['resendit'] : '';
            $email = $_POST['email'];
            ...
            if (email_exists($email) == false) {
                ...
                $user_id = $woo_slg_model->woo_slg_add_wp_user(array('email' => $email), true); // true param to indetify login via email method
                $user_flag = false;
                if ($user_id) {
    ...
    public function woo_slg_login_otp_email()
    {
        ...
        if ( isset( $_POST['otp']) && !empty($_POST['otp'])) {
            ...
            $user_id = $wpdb->get_var('SELECT user_id FROM ' . $wpdb->usermeta . ' WHERE ( meta_key = "_woo_slg_activation_code" AND meta_value = "'.$otp.'" )');
            ...
            if ($user_id) {
                ...
                if(trim($actual_otp) == trim($otp)){
                    //$activate_user = update_user_meta($user_id,'_woo_slg_user_activate','activated');
                    //update_user_meta($user_id,'_woo_slg_activation_code','');
    ...
    public function add_hooks()
    {
        ...
        if (class_exists('PeepSo')) {
            ...
            add_filter('peepso_profile_preferences', array($this->render, 'woo_slg_peepso_preference_custom_settings'), 15, 1 );
