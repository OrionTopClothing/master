...
if( !class_exists( 'WOO_Slg_Social_Instagram' ) ) {
	...
	class WOO_Slg_Social_Instagram {
		...
		public function woo_slg_initialize_instagram() {			
			...
			if ( isset( $_GET['code'] ) && !empty($_GET['code']) && isset( $_GET['wooslg'] ) && $_GET['wooslg'] == 'instagram' ) {
				...
				if( isset( $data->user ) && !empty( $data->user ) ) {
					...
					\WSL\PersistentStorage\WOOSLGPersistent::set('woo_slg_instagram_user_cache', $data->user);
				}
			}		
		}
		...
		public function woo_slg_get_instagram_user_data() {
			...
			$user_data = \WSL\PersistentStorage\WOOSLGPersistent::get('woo_slg_instagram_user_cache');
			
			\WSL\PersistentStorage\WOOSLGPersistent::delete('woo_slg_instagram_user_cache');
