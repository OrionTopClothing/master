		...
		<?php if ( $wishlist_items && count( $wishlist_items ) ) : ?>
			<?php if ( isset( $_GET['preview'] ) && $_GET['preview'] ) : ?>
                <div class="woocommerce-info woocommerce_info">
                    <a rel="nofollow" href="<?php echo $wishlist->the_url_edit(); ?>" class="button"><?php _e( 'Return to your view', 'wc_wishlist' ); ?></a> <?php _e( 'This is how other people will see your Wish List', 'wc_wishlist' ); ?>
                </div>
			<?php endif; ?>
            ...
            <table class="shop_table shop_table_responsive cart wl-table view" cellspacing="0">
				...
				if ( sizeof( $wishlist_items ) > 0 ) :
                                    ...
                                    <td class="product-purchase">
										<?php if ( ! $_product->is_type( 'external' ) && $_product->is_in_stock() && apply_filters( 'woocommerce_wishlist_user_can_purcahse', true, $_product ) ) : ?>
											<?php if ( woocommerce_wishlist_use_add_to_cart_quantity_prompts( $wishlist->id, $item ) ): ?>
                                                <a rel="nofollow" data-qty="<?php echo apply_filters( 'woocommerce_wishlist_list_item_quantity_value', esc_attr( $item['quantity'] ), $item, $wishlist ); ?>" href="<?php echo woocommerce_wishlist_url_item_add_to_cart( $wishlist->id, $wishlist_item_key, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="wishlist-add-to-cart-button-view button"><?php _e( 'Add to Cart', 'wc_wishlist' ); ?></a>
											<?php else: ?>
                                                <a rel="nofollow" href="<?php echo woocommerce_wishlist_url_item_add_to_cart( $wishlist->id, $wishlist_item_key, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="button"><?php _e( 'Add to Cart', 'wc_wishlist' ); ?></a>
											<?php endif; ?>
										<?php elseif ( $_product->is_type( 'external' ) == 'external' ) : ?>
                                            <a rel="nofollow" href="<?php echo esc_url( get_permalink( $_product->get_id() ) ); ?>" rel="nofollow" class="single_add_to_cart_button button alt"><?php echo $_product->single_add_to_cart_text(); ?></a>
										<?php endif; ?>
                                    </td>
								<?php endif; ?>
                            </tr>
                    ...
                    <tr><!-- the tds need to be individual for woothemes responsive mode, since they target the first td specifically and hide it. WIth colspan this causes issues once in responsive mode -->
						...
						<?php if ( apply_filters( 'woocommerce_wishlist_purchases_enabled', true, $wishlist ) ): ?>
                            <td class="product-purchase">
                                <a rel="nofollow" href="<?php echo woocommerce_wishlist_url_add_all_to_cart( $wishlist->id, $wishlist->get_wishlist_sharing() == 'Shared' ? $wishlist->get_wishlist_sharing_key() : false ); ?>" class="button alt wl-add-all"><?php _e( 'Add All To Cart', 'wc_wishlist' ); ?></a>
                            </td>
						<?php endif; ?>
                    </tr>
				<?php endif; ?>
